...
const ShapeCalc = () => {
  ...
  const grouped = useMemo(() => {
    ...
    for (let i = 1; i < schedule.length; i++) {
      ...
      else { groups.push(curr); curr = { interval: schedule[i].interval, count: 1 }; }
    }
    groups.push(curr);
    return groups;
  }, [schedule]);
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div className="field">
                ...
                <Stepper value={startStitches} onChange={setStartStitches} min={1} max={500} suffix="코" />
              </div>
              <div className="field">
            ...
            <div className="field">
              <label className="field-label">한 번에 변화하는 코수 <span className="field-hint">양 옆 합계</span></label>
            ...
            <div className="formula-box">
              {diff === 0 ? (
                ...
                <div>
                  ...
                  {remainder > 0 && <span style={{ color: 'var(--ink-500)' }}> · {remainder}코 나머지</span>}
                </div>
              )}
            </div>
          </div>
          ...
          {grouped.length === 0 ? (
            ...
            <>
              ...
              <div style={{ borderTop: '1px solid var(--ink-100)', paddingTop: 16 }}>
                <div style={{ fontSize: 12, color: 'var(--ink-400)', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 10 }}>
                  단별 상세
                </div>
                <div style={{ maxHeight: 240, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 4, paddingRight: 4 }}>
                  {schedule.map((s, i) => {
                    ...
                    return (
                      <div key={i} className="dist-row" style={{ borderBottom: 'none', padding: '6px 0' }}>
                        <div className="row-label">{s.idx}회차</div>
                        <div style={{ fontSize: 13 }}>
                          <strong>{s.atRow}단</strong>에서 {stitchesPerStep}코 {isDecrease ? '줄임' : '늘림'}
                        </div>
                        <div className="row-value" style={{ fontSize: 12, color: 'var(--ink-500)', fontWeight: 500 }}>
                          → {remaining}코
                        </div>
                      </div>
                    );
